...
goog.base = function(a, b, c) {
    ...
    for (var e = Array.prototype.slice.call(arguments, 2), g = !1, f = a.constructor; f; f = f.superClass_ && f.superClass_.constructor) if (f.prototype[b] === d) g = !0; else if (g) return f.prototype[b].apply(a,
        e);
...
gdocs.AnimatedBrowserIcon.prototype.animate_ = function() {
    ...
    if (a > gdocs.AnimatedBrowserIcon.MAX_ANIMATE_MSECS_) gdlog.warn("AnimatedBrowserIcon.animate", "Stopping rogue animation. numInProgress:" + this.numInProgress_ + " deltaMsecs:" + a), this.numInProgress_ = 1, this.stop(); else if (this.image_.complete) {
        ...
        chrome.browserAction.setIcon({imageData: b})
...
gdocs.TitleComputer.prototype.createTitleFromText = function(a, b) {
    ...
    var c = this.appendContentTypeExtension_(c, b), d = "";
    if (c.length > this.maxTitleLen_) {
        var e = this.splitExtension_(c), g = this.maxTitleLen_;
...
gdocs.RedirectDetector.prototype.compute_ = function(a) {
    if (a && 0 != a.byteLength) {
        ...
        var b = Math.min(a.length, gdocs.RedirectDetector.PREAMBLE_.length), b = gdocs.RedirectDetector.PREAMBLE_.substr(0, b);
...
gdocs.UrlDownloader.prototype.updatePercent_ = function(a) {
    ...
    if (a.type == gdocs.UrlDownloader.LOAD_EVENT) this.markUploadDone_(); else if (!(49 <= this.percentage_)) {
...
gdocs.UrlDownloader.prototype.onLoad_ = function(a) {
    if (!this.hasFailed()) if (this.isCancel()) this.fatal(chrome.i18n.getMessage("DOWNLOAD_CANCELED")), this.clearCancel(); else {
        var b = a.currentTarget;
        if (!this.shouldStop_(b) && !this.isTooLargeFatal_(b)) if (200 == b.status || 206 == b.status || 0 == b.status && 0 == this.url_.indexOf("file://")) {
...
gdocs.UrlDownloader.prototype.createTitle_ = function(a) {
    var b = this.getContentType_(a), c = !this.getAllowConvert() || !(b in gdocs.data.CONVERTIBLE),
        c = new gdocs.TitleComputer(gdocs.global.MAX_GENERATED_TITLE_LEN, gdocs.global.MAX_SUFFIX_LEN, c), d = this.getContentDisposition_(a);
    if (d) return c.createTitleFromText(d, b);
    d = "";
    if (a.response && 0 == b.indexOf(gdocs.MimeType.HTML)) for (var e = new Uint8Array(a.response), g = Math.min(e.length, gdocs.UrlDownloader.MAX_TITLE_SEARCH_BYTES_), f = 0; f < g; f++) {
...
gdocs.HtmlScrapeCapturer.prototype.startCapture = function() {
    ...
    chrome.tabs.executeScript(this.getTab().id, {file: this.contentScript_})
};
...
gdocs.ImageCapturer = function(a, b) {
    gdocs.Capturer.call(this, a, b, gdocs.Capturer.createFilenameFromTab(b, ".png"), !1);
    this.bg_ = a;
...
gdocs.ImageCapturer.prototype.startCapture = function() {
    chrome.tabs.captureVisibleTab(chrome.windows.WINDOW_ID_CURRENT, {format: "png"}, goog.bind(this.uploadImageUri, this))
};
...
gdocs.ImageEntireCapturer.prototype.startCapture = function() {
    chrome.tabs.executeScript(this.getTab().id, {file: "js/contentscriptscroll.js"}, goog.bind(this.initiateEntireCapture_, this));
    this.animatedBrowserIcon_.start()
};
gdocs.ImageEntireCapturer.prototype.initiateEntireCapture_ = function() {
    this.sendMessage_({msg: "init"}, goog.bind(this.processResponse_, this))
};
...
gdocs.ImageEntireCapturer.prototype.startStitch_ = function() {
    chrome.tabs.update(this.getTab().id, {active: !0}, goog.bind(this.captureVisibleAndContinue_, this))
};
gdocs.ImageEntireCapturer.prototype.captureVisibleAndContinue_ = function() {
    chrome.tabs.captureVisibleTab(chrome.windows.WINDOW_ID_CURRENT, {format: "png"}, goog.bind(this.stitchImageAndContinue_, this))
};
...
gdocs.ImageEntireCapturer.prototype.handleImageOnLoad_ = function(a) {
    if (a) {
        ...
        if (this.columnNum_ * this.visibleWidth_ >= this.canvas_.width && (this.rowNum_++, this.columnNum_ = 0, this.rowNum_ * this.visibleHeight_ >= this.canvas_.height)) {
            this.sendMessage_({msg: "restore"}, goog.bind(this.processResponse_, this));
            return
        }
        this.sendMessage_({msg: "scrollTo", x: this.columnNum_ * this.visibleWidth_, y: this.rowNum_ * this.visibleHeight_, capturePosition: this.getCornerPosition_()},
...
gdocs.ImageEntireCapturer.prototype.drawPixelsToDips_ = function(a, b, c, d, e, g, f) {
    ...
    var h = b * this.ratio_, n = c * this.ratio_, m = d * this.ratio_, p = e * this.ratio_, k = g * this.ratio_, l = f * this.ratio_;
...
gdocs.ImageEntireCapturer.prototype.getCornerPosition_ = function() {
    var a = (this.columnNum_ + 1) * this.visibleWidth_ >= this.canvas_.width, b = (this.rowNum_ + 1) * this.visibleHeight_ >= this.canvas_.height;
...
gdocs.Notify.prototype.show = function() {
    ...
    }) : goog.global.webkitNotifications && goog.global.webkitNotifications.createNotification ? goog.global.webkitNotifications.createNotification(this.iconUrl_, this.title_, this.body_).show() : goog.global.console.log("Could not display notify message:" + this.body_)
};
...
gdocs.PageCapturer.prototype.startCapture = function() {
    chrome.pageCapture.saveAsMHTML({tabId: this.getTab().id}, goog.bind(this.pageCapturedAsMhtml_, this))
};
gdocs.PageCapturer.prototype.pageCapturedAsMhtml_ = function(a) {
    if (chrome.extension.lastError) this.fatal(chrome.i18n.getMessage("FAILURE_PAGE_CAPTURE", String(chrome.extension.lastError))); else if (a) {
...
gdocs.PageCapturer.prototype.onLoadEnd_ = function(a, b) {
    if (b.target.readyState == FileReader.DONE) {
        var c = this.getTab() ? this.getTab().title : this.getFilename(), d = b.target.result.match(gdocs.PageCapturer.SUBJECT_);
...
gdocs.BrowserAction.prototype.initialize = function() {
    chrome.browserAction.setTitle({title: chrome.i18n.getMessage("SAVE_TO_GOOGLE_DRIVE_ACTION")});
...
goog.string.compareVersions = function(a, b) {
    for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), g = Math.max(d.length, e.length), f = 0; 0 == c && f < g; f++) {
        var h = d[f] || "", n = e[f] || "", m = RegExp("(\\d*)(\\D*)", "g"), p = RegExp("(\\d*)(\\D*)", "g");
        do {
            var k = m.exec(h) || ["", "", ""], l = p.exec(n) || ["", "", ""];
            if (0 == k[0].length && 0 == l[0].length) break;
            var c = 0 == k[1].length ? 0 : parseInt(k[1], 10), q = 0 == l[1].length ? 0 : parseInt(l[1], 10), c = goog.string.compareElements_(c, q) || goog.string.compareElements_(0 ==
...
goog.asserts.doAssertFailure_ = function(a, b, c, d) {
    ...
    if (c) var e = e + (": " + c), g = d; else a && (e += ": " + a, g = b);
    ...
    chrome.identity.getAuthToken({interactive: !0}, function(m) {
        if (chrome.runtime.lastError) gdlog.infoLastErr("Client.authAndSend", "getAuthToken"), h(chrome.runtime.lastError.message || "N/A"); else {
            ...
            k.onreadystatechange = function() {
                if (4 == k.readyState) if (g && k.status == gdocs.HttpStatus.UNAUTHORIZED) gdlog.info("Client.authAndSend", "Retry. responseText:" + k.responseText + " errMsg:" + gdocs.msgutil.createXhrErrMsg(k)),
                    goog.asserts.assert(m), chrome.identity.removeCachedAuthToken({token: m}, function() {
                    gdocs.Client.authAndSend_(a, b, c, d, e, !1, f, h)
                }); else if (-1 != p.indexOf(k.status)) f(k); else {
...
gdocs.ContextMenu = {};
gdocs.ContextMenu.ContextType = {PAGE: "page", LINK: "link", IMAGE: "image", VIDEO: "video", AUDIO: "audio"};
gdocs.ContextMenu.createMenus = function(a) {
    for (var b = [[gdocs.ContextMenu.ContextType.LINK, "SAVE_LINK"], [gdocs.ContextMenu.ContextType.IMAGE, "SAVE_IMAGE"], [gdocs.ContextMenu.ContextType.VIDEO, "SAVE_VIDEO"], [gdocs.ContextMenu.ContextType.AUDIO, "SAVE_AUDIO"]], c = 0; c < b.length; c++) {
        var d = b[c];
...
gdocs.ContextMenu.createMenuClickCallback_ = function(a) {
    return function(b, c) {
        var d = gdocs.ContextMenu.getUserImpression_(b), e = gdocs.ContextMenu.getClickUrl_(b), g = gdocs.ContextMenu.createDefaultFilenameFromMediaType_(b),
...
gdocs.ContextMenu.createMenu_ = function(a, b, c) {
    ...
    c = chrome.contextMenus.create({title: a, contexts: [b], onclick: c});
...
gdocs.Options.StorageKeys_ = {HTML_FORMAT: "htmlFormat", CONVERT_TO_GOOGLE_FORMAT: "convertToGoogleFormat", USER_FOLDER_MAP: "userFolderMap"};
...
gdocs.TabContentType = function() {
    ...
    chrome.webRequest.onResponseStarted.addListener(goog.bind(this.responseStartedCallback_, this), {urls: ["<all_urls>"], types: ["main_frame"]}, ["responseHeaders"])
};
...
gdocs.UserId.prototype.getUserIdStr = function(a) {
    this.currentId_ ? a(this.currentId_) : chrome.identity.getAuthToken({interactive: !1}, goog.bind(this.userIdAuth_, this, a))
};
...
gdocs.BackgroundPage.prototype.showUploadDialog_ = function(a) {
    if (a) {
        ...
        chrome.windows.create({url: chrome.extension.getURL("upload.html"), type: "popup", width: a, height: b})
...
gdocs.BackgroundPage.prototype.displayWhenAuthorized = function() {
    ...
    chrome.identity.getAuthToken({interactive: !0}, goog.bind(this.showUploadDialog_, this))
};
